  ...
  : myPhysicsList(myPL)
{
  ...
  if (file_types&1)
    file.open("./Bina_pd_out1.dat");
  if (file_types&2)
    file2.open("./Bina_pd_out2.dat");
  if (file_types&4)
    file3.open("./Bina_pd_out3.dat");
  int i;
...
void Bina_SteppingAction::UserSteppingAction(const G4Step * theStep)
{
  ...
  static int first = 1, once = 1, ilosc = 0, next = 0,ilosc2 = 0,
             firsts = 1, bound = 0, ebound = 0;
...
//G4cout<<(index1++)<<' '<<(index0++)<<'\n';
  ...
  if (tab[1])
  {
    ...
    if(thePrePVname(0,8)=="Target_p"&&theTrack->GetParentID()==0)
    {
      ...
      if (npd_choice < 0) i=0;
      else if ((npd_choice < 8) && !(npd_choice == 2) && !(npd_choice == 5))
      {
        if (tab[1]==2) i=0;
	else if (tab[1]==3) i=1;
      ...
      //----------ISCh-----
      //      else if ((npd_choice > 2)&&(npd_choice < 5))
      //      {
      //       if      (fabs(energy[0] - startEnergy)<0.001) i = 0;
      //       else if (fabs(energy[1] - startEnergy)<0.001) i = 1;
      //       else G4cout << std::setw(24) <<"error -> " <<startEnergy<<G4endl;
