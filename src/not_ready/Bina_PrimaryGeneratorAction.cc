...
//  set particles mass
  ...
  if (npd_choice <  0)
  {
    ...
    {
      G4cout << "Error in configuration file Bina_Gun.cfg !!! Bad npd_choice = "<<npd_choice<<G4endl;
      exit(1);
    }
  ...
  else if ((npd_choice == 3)||(npd_choice == 4))
  {
    if (npd_choice == 4) 
    ugelast_read();  //read cross table
    particleGun1 = new G4ParticleGun(n_particle);
    particleGun2 = new G4ParticleGun(n_particle);
    ...
    G4ParticleDefinition* particle2 = particleTable->FindParticle("deuteron");
    //    particle = particleTable->FindParticle("deuteron");
    particleGun2->SetParticleDefinition(particle2);
  }
  ...
  else if ((npd_choice == 6)||(npd_choice == 7))
  {
    if (npd_choice == 7) ugelast_read();  //read cross table
    particleGun1 = new G4ParticleGun(n_particle);
    particleGun2 = new G4ParticleGun(n_particle);
...
{
  delete particleGun1;
  if ((npd_choice == 0)||(npd_choice == 1)||(npd_choice == 3)||(npd_choice == 4)) delete particleGun2;
										  if ((npd_choice == 6)||(npd_choice == 7)) delete particleGun1;
  if ((npd_choice == 2)||(npd_choice == 5)||(npd_choice == 8)) delete particleGun3;
  //  if ((npd_choice == 3)||(npd_choice == 4)) delete particleGun2;
  //  if (npd_choice == 5 ) delete particleGun3;
}
...
void Bina_PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
  ...
  else if ((npd_choice == 0)||(npd_choice == 3)||(npd_choice == 6))
  {
    //G4cout <<"Npd_choice : "<<npd_choice<<G4endl;
    upunif(momentum);
    ProcNb(npd_choice);
  ...
  else if ((npd_choice == 1)||(npd_choice == 4)||(npd_choice == 7))
  {
    //G4cout <<"Npd_choice : "<<npd_choice<<G4endl;
    ugelast(momentum);
    ProcNb(npd_choice);
  ...
  else if (npd_choice == 4)
  {
    ...
    ProcNb(npd_choice);
    Pos();
    //       cout<<"TUUU  Energy1 :"<<bt1;
    //       cout<<"TUUU  energy2:"<<bt2;
    GetStartEnergy(bt1*1000.,bt2*1000.);
    GetStartAngleTheta(t1,t2);
    GetStartAnglePhi(fi1,fi2);
    GetStartPosition(vertex[0],vertex[1],vertex[2]);
  ...
  else if ((npd_choice == 2)||(npd_choice == 5)||(npd_choice == 8))
  {
    //filell.open("fileoutput.txt",ios::out|ios::app);
    //G4cout <<"Npd_choice : "<<npd_choice<<G4endl;
    ugbreak(momentum);
    ...
    particleGun1->SetParticleEnergy(bt1*GeV);
    particleGun2->SetParticleEnergy(bt2*GeV);
    particleGun3->SetParticleEnergy(bt3*GeV);
...
double* Bina_PrimaryGeneratorAction::elastic(double *ptot)
{
  ...
  thcosm = cos(themax*pi/180.);
  thcos0 = cos(themin*pi/180.);
  thcos = thcosm + (thcos0 - thcosm)*RandomFlat(aj1theta);
  t1 = 180./pi*acos(thcos);
  pphi = pi*(2.* RandomFlat(aj1phi));
  fi1=pphi*180./pi;
...
double* Bina_PrimaryGeneratorAction::upunif(double *ptot)
{
  ...
  double thcosm,thcos0,thcosm2,thcos02,thcos,thcos2,pphi; // auxillary variable
  double *w_bp1,*w_bp2,pm,dm;
  ...
  thcosm = cos(themax*pi/180.);
  thcos0 = cos(themin*pi/180.);
  thcosm2 = cos(themax2*pi/180.);
  thcos02 = cos(themin2*pi/180.);
  ...
  ptot2[0] = bpproj*cos(pi + pphi);
  ptot2[1] = bpproj*sin(pi + pphi);
...
//  double csmax = 15.;
  ...
  {
    ok = 1;
    thelab = pi*0.5*RandomFlat(aj1theta);
    thcos = cos(thelab);
...
// p(d,pp)n (d(p,pp)n) break-up cross section
...
// offs(i) - offsets of the cross section tables
  ...
  float ya[4][4][4][4], yax[4][4][4][4];
  float yay[4][4][4][4],yaxx[4][4][4][4],yaxy[4][4][4][4],yayy[4][4][4][4];
  double e_random;
...
*/
  do
  {
    ...
    theta1 = 180./pi*acos(thcos1);
    theta2 = 180./pi*acos(thcos2);
...
// calculate momenta of p, p and n
  thcos3 = cos(t3*pi/180.);  //rs - change t1 -> t3
  bp1 = bp[0];
  bp2 = bp[1];
  bp3 = bp[2];
...
{  // output : bp1,bp2,thcos2
  ...
  //  double dm = d_mass;
  // double pm = p_mass;
  if ((npd_choice == 3)||(npd_choice == 4)||(npd_choice == 6)||(npd_choice == 7))
    {
      pm = d_mass;
    }
  else
...
float Bina_PrimaryGeneratorAction::dpb_kin3(const int typ, double phi12,double *wphi13,double e2_0,double* we1_0,double* we3_0,double *bp_t)
{
...
float Bina_PrimaryGeneratorAction::rinterp(float ytmp[5][5][5][5],float x1,float x2, float x3, float x4)
{
//#include"fstream.h"
////#include"G4ios.hh"
//#include"iostream.h"
...
// x4  - kinetic energy table
  ...
  float /*x1=0.,x2=0.,x3=0.,x4=0.,*/y=0.;//10IV2010
  int m1=4,m2=4,m3=4,m4=4;
  int iiii,jjjj,kkkk,llll;
  float *ya = &ytmp[0][0][0][0];
  float *y3a = &y_t1[0][0][0][0];
  float *w_y = &y;

  const float *ya_const, *y3a_const;
//  filell.open("fileoutput.txt",ios::out|ios::app);
  ya_const = ya;	
  y3a_const = y3a;
...
void Bina_PrimaryGeneratorAction::spline4(float */*x1a*/,float */*x2a*/,float */*x3a*/,float *x4a,float *ya,
					int m1,int m2,int m3,int m4,float *y3a)
{
  const float *ya_const, *y3a_const, *x4a_const;
  ya_const = ya;
  y3a_const = y3a;
  x4a_const = x4a;
  ...
  for (int j=1;j<=m1;j++)
  {
    ...
    for (int k=1;k<=m2;k++)
    {
      ...
      for (int l=1;l<=m3;l++)
      {
	spline(x4a,ya,m4,1.0e30,1.0e30,y3a);
	ya  +=5;
        y3a +=5;
      }
    }
...
void Bina_PrimaryGeneratorAction::splint4(float* x1a,float* x2a,float* x3a,float* x4a,float *ya,float *y2a,
					int m1,int m2,int m3,int m4,float x1,float x2,float x3,float x4,float *y)
{
  float *ytmp,*yytmp,*yyytmp;
  float *ytmp1,*yytmp1,*yyytmp1;
  const float *ya_const,*y2a_const;
  int jjj;
  ya_const = ya;
  y2a_const = y2a;
  ...
  for (int j=1;j<=m1;j++)
  {
    ...
    for (int k=1;k<=m2;k++)
    {
      ...
      for (int l=1;l<=m3;l++)
      {
        splint(x4a,ya,y2a,m4,x4,&yytmp[l]);
        ya  += 5;
        y2a +=5;
       // for (jjj=1;jjj<=4;jjj++) filell<<"1-  "<<l <<' '<<ya[jjj]<<' '<<y2a[jjj]<<' '<<yytmp[jjj]<<G4endl;
        }
      spline(x3a,yytmp,m3,1.0e30,1.0e30,ytmp);
      splint(x3a,yytmp,ytmp,m3,x3,&yyytmp[k]);
    //   for (jjj=1;jjj<=4;jjj++) filell<<"2- "<<k<<' '<<yytmp[jjj]<<' '<<ytmp[jjj]<<' '<<yyytmp[jjj]<<G4endl;
      }
    spline(x2a,yyytmp,m2,1.0e30,1.0e30,yytmp1);
    splint(x2a,yyytmp,yytmp1,m2,x2,&ytmp1[j]);
//     for (jjj=1;jjj<=4;jjj++) filell<<"3- "<<j<<' '<<yyytmp[jjj]<<' '<<yytmp1[jjj]<<' '<<ytmp1[jjj]<<G4endl;
    }
  spline(x1a,ytmp1,m1,1.0e30,1.0e30,yyytmp1);
  splint(x1a,ytmp1,yyytmp1,m1,x1,y);
...
void Bina_PrimaryGeneratorAction::splint(float* xa,float* ya,float* y2a,int n,float x,float *y)
{
...
void Bina_PrimaryGeneratorAction::spline(float x[], float y[], int n, float yp1, float ypn, float *y2)
{
...
float *Bina_PrimaryGeneratorAction::vector(long nl, long nh)
...
void Bina_PrimaryGeneratorAction::free_vector(float *v, long nl, long /*nh*/)
