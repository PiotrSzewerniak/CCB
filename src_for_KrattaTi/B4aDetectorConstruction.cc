...
//G4VSolid* kr_housingS2b1 = new G4Trd("kr_housing2b1",fKRhouse_xy1-housThick1,fKRhouse_xy2-housThick1,fKRhouse_xy1-housThick1,fKRhouse_xy2-housThick1,fKRhouse_z);
  ...
  */
  ...
  //G4double spacing=(10.*mm+fPd_xy+2.*frameWidth)/(dist+fKRhouse_z+frameThick);
  //G4double spacing=(10.*mm+49.*mm)/(dist+fKRhouse_z+frameThick);
  G4double spacing=8.5*mm;
  G4double pSPhi2=135.*pi/180.;
  G4double pDPhi2=90*pi/180.;
  G4int    noDet_ring=4; 
  //G4double widthKr1=2*(fPd_xy+frameWidth)/dist; 
  G4double widthKr1=49.*mm/(dist+fKRhouse_z+frameWidth); 
  
  G4double thCenter=90.*pi/180.;
  G4int noDet=2;
  //closer to the beam pipe
  G4double phi_det=5;
  G4double phiMinL=(3./2.)*pi+phi_det*pi/180.+noDet*widthKr1;
  ...
  for (G4int cNt=-2;cNt<=2;cNt++)
      ...
      for (G4int cN=0;cN<noDet;cN++)
	{
	  ...
	  if (cN==0){
	    phi= phiMinL-cN*widthKr1;  
	    phi_r= phiMinR+cN*widthKr1; 
    
	  }
	  else {
	    phi= phiMinL-cN*widthKr1-spacing/(dist+fKRhouse_z);
	    phi_r= phiMinR+cN*widthKr1+spacing/(dist+fKRhouse_z);
	  }
  
   

    //THETA
    //G4double theta= thCenter+cNt*(widthKr1+2.*(fPd_xy+frameWidth)/ftar_det); 
    //G4double theta= thCenter+cNt*(24.5*mm/ftar_det); 
    G4double theta=0.;
    if (cNt<0)theta=thCenter+(cNt+1)*5.5*pi/180.-2.75*pi/180.;
    else theta=thCenter+(cNt-1)*5.5*pi/180.+2.75*pi/180.;
    ...
    rm1.rotateX(0*deg);
    rm1.rotateY(90*deg+alpha+betha);
    rm1.rotateZ(phi);
